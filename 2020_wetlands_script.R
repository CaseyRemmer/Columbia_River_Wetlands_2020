...
dt_2020<-read.csv("raw data/CRW_2020_raw data master_SB checked.csv")
head(dt_2020)
...
red is (255, 0,0) groundwater
green is (0, 255, 0) river
blue is (0,0, 255) rain

colr_list<-list()
for (i in 1:3){
  colr<-matrix(nrow=nrow(results_2020[[i]]))
  for (n in 1:nrow(results_2020[[i]])){
    colr[n,]<-rgb2hex(
      r=round(results_2020[[i]]$gw_mean[n]*255, 0),
      g=round(results_2020[[i]]$river_mean[n]*255,0),
      b=round(results_2020[[i]]$precip_mean[n]*255,0))}
  colr_list[[i]]<-colr}
